...
- **Row-Level Security testing on personal tenants:** implemented a `Gauteng_RiskManager` role restricting `dim_customers[Province] = 'Gauteng'`, with the filter correctly propagating to `fact_loans` and `fact_repayments` via existing relationships. Live validation via "View as role" or role member assignment requires either Power BI Desktop or an organizational Microsoft Entra directory to search for accounts — both unavailable on a personal/trial Fabric tenant. The role was validated through its DAX logic and relationship design rather than a live preview, since the testing convenience (not the security feature itself) is what's tenant-restricted.
